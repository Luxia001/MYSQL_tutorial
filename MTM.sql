...
select * from reviewers,series,reviews;
...
select first_name,last_name,ifnull(count(rating),0),ifnull(min(rating),0),ifnull(max(rating),0),ifnull(round(avg(rating),2),0),
...
join reviewers on reviewers.id = series.id;
